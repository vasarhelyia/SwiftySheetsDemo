...
#import "ASTAddExpression.h"

@interface ASTAddExpression ()
...
@implementation ASTAddExpression
...
- (ASTExprType)type {
	...
	} else if(self.operand1.type == ASTExprType_String && self.operand2.type == ASTExprType_String) {
...
- (id)eval {
	if(self.operand1.type == ASTExprType_Numeric && self.operand2.type == ASTExprType_Numeric) {
		...
		return [(NSString *)self.operand1.eval stringByAppendingString:(NSString *)self.operand2.eval];
	} else {
		@throw [NSError errorWithDomain:@"ParserErrorDomain" code:-3 userInfo:@{NSLocalizedDescriptionKey :  @"Invalid operand types"}];
	}
}
